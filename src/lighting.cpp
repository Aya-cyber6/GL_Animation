...
bool LightingTechnique::Init(int SubTech)
{
    ...
    case SUBTECH_DEFAULT:
        if (!AddShader(GL_VERTEX_SHADER, "Shaders/lighting_new.vs")) {
            return false;
        }
    ...
    case SUBTECH_PASSTHRU_GS:
        if (!AddShader(GL_VERTEX_SHADER, "Shaders/lighting_new_to_gs.vs")) {
            return false;
        }

        if (!AddShader(GL_GEOMETRY_SHADER, "Shaders/passthru.gs")) {
            return false;
        }
        break;
    ...
    case SUBTECH_WIREFRAME_ON_MESH:
        ...
        if (!AddShader(GL_GEOMETRY_SHADER, "Shaders/wireframe_on_mesh.gs")) {
            return false;
        }
        break;
